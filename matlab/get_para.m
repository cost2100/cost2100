...
%------
%network :
%'IndoorHall_5GHz','SemiUrban_300MHz','SemiUrban_CloselySpacedUser_2_6GHz', or 'SemiUrban_VLA_2_6GHz'
...
switch network
    case 'SemiUrban_300MHz'
        ...
        paraEx.BS_range = 1; % Simulated base station spatial range [m], for VLA
    ...
    case 'SemiUrban_VLA_2_6GHz'
        ...
        paraEx.n_floor = 0; % Number of floors between BS and MS
        paraEx.BS_range = 8; % Simulated base station spatial range [m], for VLA
end
...
switch paraEx.network           
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%
        ...
        paraSt.mpc_gain_sigma = Inf; % Inf means gain function feature has no effect
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        switch paraEx.scenario
            ...
            %%%%%%%%%%%%%%%%
                ...
                paraSt.mpc_gain_sigma = sqrt(-1*r_mpc_gain^2/(2*log(0.7))); % Sigma of MPC Gaussian gain function 
            ...
            %%%%%%%%%%%%%%%%
                ...
                paraSt.mpc_gain_sigma = 1.2; % Sigma of MPC gain function 
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
         switch paraEx.scenario
            ...
            %%%%%%%%%%%%%%%%
                paraSt.bs_vr_mu = 0.7; % BS-VR length, lognormal, mu
                paraSt.bs_vr_sigma = 2; % BS-VR length, lognormal, sigma                
                paraSt.bs_vr_slope_mu = 0; % -0.21 -> 0; % BS-VR power slope, normal, mu
                ...
                nCluster_para_r = 2.43;
                nCluster_para_p = 0.16;
                n_c_far = nbinrnd(nCluster_para_r, nCluster_para_p); % Negative binominal 
                paraSt.n_c_far = n_c_far; % Average number of far clusters                
                ...
                paraSt.n_mpc = 100; % Number of MPCs per cluster
                r_mpc_gain = 2; % 3 dB power decay in meters
                paraSt.mpc_gain_sigma = sqrt(-1*r_mpc_gain^2/(2*log(0.7))); % Sigma of MPC gain function 
            ...
            %%%%%%%%%%%%%%%%
                ...
                nCluster_para_r = 2.43; % Total number of clusters parameter
                nCluster_para_p = 0.16; % Total number of clusters parameter
                ...
                r_mpc_gain = 100; % 3 dB power decay in meters
                paraSt.mpc_gain_sigma = sqrt(-1*r_mpc_gain^2/(2*log(0.7))); % Sigma of MPC gain function 
